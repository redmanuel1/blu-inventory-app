...
export class InventoryComponent implements OnInit {
  ...
  inventory: Inventory[] = [];
  // Will be used for inventory transaction to tell how many were added or removed from the inventory
  ...
  ngOnInit() {
    ...
    this.recordService.getRecords().subscribe((data) => {
      this.inventory = data.sort(
        (a, b) =>
          new Date(b.dateUpdated).getTime() - new Date(a.dateUpdated).getTime()
      );
      if (this.oldInventory.length == 0) {
        this.oldInventory = structuredClone(data);
      }
  ...
  async saveInventory(records: any[]) {
    ...
    try {
      ...
      const { updates, newRecords } = records.reduce(
        (acc, record) => {
        ...
        if (duplicateExists.length === 0) {
          ...
          duplicateExists.forEach((doc) => {
            ...
            const updatedQuantity =
              (existingData.quantity || 0) + (record.quantity || 0);
      ...
      await this.processInventoryTransaction(
        recordTobeAdded,
        duplicateRecords,
        updates
      );
  ...
  async deleteInventory(record: any) {
    this.recordService.collectionName = "Inventory";
    this.recordService
      .deleteRecord(record.id)
      .then(() => {
        this.hideSpinnerAddToast(
          "Inventory deleted : " + record.name,
          "success"
        );
      })
      .catch((error) => {
        this.hideSpinnerAddToast(
          "Error deleting product " + record.name + " : " + error,
          "error"
        );
      });
    // Add an inventory transaction
    const inventoryTransactionArr = this.getInventoryTransaction(
      [record],
      "DELETE"
    );
    this.recordService.collectionName = "InventoryTransaction";
    await this.recordService.addRecords(inventoryTransactionArr);
  }

  async processInventoryTransaction(
    addedRecordArr: Inventory[],
    duplicateRecordArr: Inventory[],
    updatedRecordArr: Inventory[]
  ) {
    console.log(this.oldInventory);
    const addInventoryTransactionArr: InventoryTransaction[] =
      this.getInventoryTransaction(addedRecordArr, "ADD");
    const duplicateInventoryTransactionArr: InventoryTransaction[] =
      this.getInventoryTransaction(duplicateRecordArr, "ADD");
    const updateInventoryTransactionArr: InventoryTransaction[] =
      this.getInventoryTransaction(updatedRecordArr, "ADD");

    this.recordService.collectionName = "InventoryTransaction";
    const inventoryTransactionArr = [
      ...addInventoryTransactionArr,
      ...duplicateInventoryTransactionArr,
      ...updateInventoryTransactionArr,
    ];
    await this.recordService.addRecords(inventoryTransactionArr);
  }

  getInventoryTransaction(inventoryArr: Inventory[], type: "ADD" | "DELETE") {
      ...
      const inventoryTransaction: InventoryTransaction = {
        ...
        amount:
          type == "DELETE"
            ...
            : inventory.price * inventoryQty,
        inOrOut: type == "DELETE" ? "OUT" : inventoryQty < 0 ? "OUT" : "IN",
        quantity: type == "DELETE" ? inventory.quantity : inventoryQty,
        receivedFromOrGivenTo: this.authService.getUserIdNo(),
    ...
    return inventoryTransactionArr;
  }
